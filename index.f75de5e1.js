...
firstPromise.then(()=>createMessage("First promise was resolved!", "success")).catch(()=>createMessage("First promise was rejected!", "error"));
const secondPromise = new Promise((resolve, reject)=>{
    document.addEventListener("click", (e)=>{
        if (e.button === 0 && !(e.button === 2)) resolve();
    });
...
const thirdPromise = new Promise((resolve)=>{
    document.addEventListener("mousedown", (e)=>{
        ...
        if (leftButtonPressed && rightButtonPressed) resolve();
    });
});
thirdPromise.then(()=>{
